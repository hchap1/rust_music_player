...
use crate::playlist::is_song_downloaded;
...
impl DownloadTask {
    pub fn from_file(filepath: &String, programdir: String) -> Result<Self, String> {
        match read_to_string(filepath) {
            Ok(raw) => Ok(Self { queries: raw.lines().map(|x| x.to_string()).collect::<Vec<String>>(), directory: programdir }),
    ...
    pub async fn download(&self) -> Result<String, Vec<String>> {
        ...
        let futures: Vec<_> = self.queries.iter().map(|x| download_query(self.directory.as_str(), x, Arc::clone(&errors))).collect::<Vec<_>>();
